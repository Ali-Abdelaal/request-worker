 ...
 * @param bindingTarget the ip that the socket should connect on it, like 'tcp://127.0.0.1:3000'
 */
worker.init = function (bindingTarget) {
    try {
        _socket.connect(bindingTarget);
    }
    catch (error) {
        winston.log('info', error);
        throw new Error("Fail to connect " + bindingTarget);
    }
    winston.log("info", 'Worker connected to %s', bindingTarget);
};
